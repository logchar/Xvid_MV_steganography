...
int main(int argc, char *argv[])
{
	...
	/////////////////// 添加传递参数进行初始化 //////////////////////
	ExPara.iHitRate = 0;
	ExPara.pMesFileName = NULL;
	ExPara.iSumBitOffset = 0;
	ExPara.iMesLen = 0;
	ExPara.iKey = 0;
	ExPara.Dump[0]= 0;
	ExPara.iSranded = 0;
 ...
 ****************************************************************************/
	...
	for (i=1; i< argc; i++) {
		...
		if (strcmp("-noasm", argv[i]) == 0 ) {
			...
			usage();
			return(0);
		} else if (strcmp("-mes", argv[i]) == 0 && i < argc - 1 ) { // -mes 表示 message，当存在时需要提取秘密文件
			i++;
			ExPara.pMesFileName = argv[i];
		}else if (strcmp("-key", argv[i]) == 0 && i < argc - 1) { // -key 表示提取的密钥
			i++;
			ExPara.iKey = atoi(argv[i]);
		}else {
			usage();
			exit(-1);
		}
...
/*****************************************************************************
 * Values checking   进行参数取值的检查
 ****************************************************************************/

//===========  添加文件打开操作、进行嵌入随机函数种子初始化 ============

	if(ExPara.pMesFileName)	//
	{
		Mfp = fopen(ExPara.pMesFileName, "w+");
	...
	{
		printf("<<< Will not extract secret message.\n");
	}
	//秘密信息文件

	printf("<<< Extract key : %d\n", ExPara.iKey);
 ...
 ****************************************************************************/
	...
	status = dec_init(use_assembler, debug_level, NULL);
	if (status) {
		fprintf(stderr,
 ...
 ****************************************************************************/
	...
	do {
		...
		do {
			...
			/* Decode frame */
			dectime = msecond();
			used_bytes = dec_main(mp4_ptr, out_buffer, useful_bytes, &xvid_dec_stats,&ExPara);
		...
		if (!display_buffer_bytes) {
			...
			if(ExPara.iSumBitOffset<=ExPara.iMesLen*8)
				printf("AMC-[%d%%]%d/%d completed.Hit=%d C=%d\n",ExPara.iSumBitOffset*100/8/(ExPara.iMesLen+2),ExPara.iSumBitOffset/8,ExPara.iMesLen+2,ExPara.iHitRate,ExPara.iSm);
			//添加提取进度
		}
 ...
 ****************************************************************************/
	...
	do {
        ...
        do {
		    dectime = msecond();
		    used_bytes = dec_main(NULL, out_buffer, -1, &xvid_dec_stats, &ExPara);
		    dectime = msecond() - dectime;
			if (display_buffer_bytes) {
				printf("Data chunk %d: %d bytes consumed, %d bytes in buffer\n", chunk++, used_bytes, useful_bytes);
			}
...
static int
dec_init(int use_assembler, int debug_level,ExtractPara* pExPara)//添加提取参数
{
	int ret;
	...
	ret = xvid_decore(NULL, XVID_DEC_CREATE, &xvid_dec_create, pExPara);
		 ...
		 ExtractPara* pExPara)//添加提取参数
{
	...
	ret = xvid_decore(dec_handle, XVID_DEC_DECODE, &xvid_dec_frame, xvid_dec_stats, pExPara);
...
static int
dec_stop(ExtractPara* pExPara)
{
	int ret;

	ret = xvid_decore(dec_handle, XVID_DEC_DESTROY, NULL, NULL, pExPara);
