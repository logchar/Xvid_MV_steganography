 ...
 *	
 ************************************************************************/
#include <io.h>//添加 filelength函数 头文件
...
#include "xvid.h"
#include "global.h"	//添加全局变量头文件（包含了我们定义的参数）
//#include "XYB.h"	//添加
...
static int  enc_main(void *enc_handle,
					 ...
					 int stats[3],
					 int framenum,
					 EmbeddingPara* pEmPara);
static void encode_sequence(enc_sequence_data_t *h,EmbeddingPara* pEmPara) ;//添加嵌入参数
...
int GetSm(EmbeddingPara* pEmPara,int rate,int sum)
{
	//根据百分比rate，求出阈值sm
	
	int num=(sum*rate)/100;
	//求出总宏块个数sum中，需要使用的宏块数目num

	int i=0;		//计数
	int pre_sum=0;	//前面若干sm数目总和

	for(i=0;i<10000;i++)
	{
		pre_sum+=pEmPara->SmDump[i];

		if(pre_sum>num)
		{
			pEmPara->iBlockNum=pre_sum;
			return i;
		}
	}
	pEmPara->iBlockNum=pre_sum;
...
{
	//================ 下面是我们添加的全局参数接收变量 ==============
	EmbeddingPara EmPara;
	FILE* Mfp;	//秘密信息文件指针
	enc_sequence_data_t data_backup;
	//=======================================================
...
//================== 下面是我们添加的全局参数初始化 ==================
	EmPara.iKey = 0;			//嵌入密钥
	Mfp = NULL;					//秘密信息文件指针
	EmPara.iSumBitOffset = 0;	//所有嵌入信息的序列bit偏移标记
	EmPara.iBlockNum = 0;				//可用于嵌入的大宏块数量
	EmPara.iSumBlockNum = 0;			//所有大宏块数量
	EmPara.iFirstTime = 1;		//第一次进行嵌入容量预估
	EmPara.iEmRate = 10;				//默认绝对嵌入率1%
//=========================================================
 ...
 ****************************************************************************/
	...
	for (i = 1; i < argc; i++) {
		if (strcmp("-asm", argv[i]) == 0) {
			...
			usage();
			return (0);
		} else if (strcmp("-mes", argv[i]) == 0 && i < argc - 1 ) { // -mes 表示 message，当存在时需要嵌入秘密文件
			i++;
			EmPara.pMesName = argv[i];
		}else if (strcmp("-key", argv[i]) == 0 && i < argc - 1) {// -key 表示嵌入的密钥
			i++;
			EmPara.iKey = atoi(argv[i]);
		}else if (strcmp("-emrate", argv[i]) == 0 && i < argc - 1) {// -emrate 表示绝对嵌入率取值 1 2 3 ... 
			i++;
			EmPara.iEmRate = atoi(argv[i]);
		}else {
			usage();
			exit(-1);
		}
...
/*****************************************************************************
 *                            Arguments checking  参数合法性检查
 ****************************************************************************/

//=========== 添加文件打开操作、进行嵌入随机函数种子初始化 ============

	if(EmPara.pMesName)	//如果有指定要嵌入的信息文件
	{
		Mfp = fopen(EmPara.pMesName, "rb+");
		...
		{
			EmPara.iMesLen = filelength  (fileno(Mfp)); 
			printf(">>> '%s' opened successfully.\n", EmPara.pMesName);
		}
	}
	else
	{
		printf(">>> Will not embed secret message.\n");
	}
	
	if(EmPara.iKey==0)	//如果KEY是0，即是默认的密钥
	{
		printf(">>> Use default key : 0\n");
	}
	else	//否则此时的key则是用户自定义的
	{
		printf(">>> Embedding Key : %d\n", EmPara.iKey);
	}
...
//==========================================================
...
#endif /* Spawn just one encoder instance */
	{
		...
		data_backup = enc_data;
		//备份下

		EmPara.SmDump=(int*)malloc(10000*sizeof(int));
		for(i=0;i<10000;i++)
			EmPara.SmDump[i]=0;
		//SmDump清零

		encode_sequence(&enc_data,&EmPara);
		//添加第一次调用，嵌入容量估计，此时Sm的分布律存在于EmPara.SmDump中

		EmPara.iFirstTime = 0;
		//第二次进行真正嵌入

		EmPara.iMesLen = EmPara.iSumBlockNum * EmPara.iEmRate / 800 *2;
		//计算需要嵌入信息的字节数

		EmPara.InfDump=(unsigned char*)malloc(6+EmPara.iMesLen);
		//根据需要，开辟一个足够大的空间，存放即将嵌入的秘密信息

		if(EmPara.InfDump)//内存分配成功
		{
			EmPara.iMesLen = fread(&EmPara.InfDump[6],1,EmPara.iMesLen,Mfp);
			//将信息文件一次读入内存，根据读取的字节数设为需要嵌入的信息长度

			EmPara.ThresholdSm=GetSm(&EmPara,EmPara.iEmRate,EmPara.iSumBlockNum);
			//获取SM阈值

			//EmPara.Sm=(EmPara.Sm+1)*1.1;

			EmPara.iBlockNum=0;
			for(i=0;i<=EmPara.ThresholdSm;i++)
				EmPara.iBlockNum+=EmPara.SmDump[i];
			
			//EmPara.iHitRate = EmPara.iSumBlockNum * EmPara.iEmRate * EmPara.iBlockNum/100;
			EmPara.iHitRate = EmPara.iMesLen*800/EmPara.iBlockNum/2;
			
			if(EmPara.iHitRate>95)
				EmPara.iHitRate = 100;

			printf("\nSum Block Num = %d Block Num = %d Hit=%d SM = %d\n",EmPara.iSumBlockNum,EmPara.iBlockNum,EmPara.iHitRate,EmPara.ThresholdSm);

			enc_data = data_backup;
			//备份还原，准备二次调用

			EmPara.iCompleted = 0;
			//表示未嵌入

			EmPara.InfDump[0] = EmPara.iHitRate;
			//命中率存入参数

			EmPara.InfDump[1] = EmPara.iMesLen/65536;
			EmPara.InfDump[2] = (EmPara.iMesLen%65536)/256;
			EmPara.InfDump[3] = EmPara.iMesLen%256;
			//信息长度嵌入参数

			EmPara.InfDump[4] = EmPara.ThresholdSm/256;
			EmPara.InfDump[5] = EmPara.ThresholdSm%256;
			//sm嵌入参数
		}
		else
		{
			printf("ERROR：Can not allocate enough memorry for embedding!\n");
			if(Mfp)fclose(Mfp);
			getch();
			return -1;
		}
...
void encode_sequence(enc_sequence_data_t *h,EmbeddingPara* pEmPara) {//添加嵌入参数
 ...
 ****************************************************************************/
		...
		m4v_size =
			enc_main(enc_handle, !result ? in_buffer : 0, mp4_buffer, &key, &stats_type,
					 &stats_quant, &stats_length, sse, input_num,pEmPara);
...
#define SSE2PSNR(sse, width, height) ((!(sse))?0.0f : 48.131f - 10*(float)log10((float)(sse)/((float)((width)*(height)))))
			...
			if (ARG_PROGRESS == 0) {
				...
				printf(" | type=%s, quant= %2d, len= %7d", type, stats_quant,
				   stats_length);
				if(pEmPara && pEmPara->iFirstTime == 0 && pEmPara->iCompleted ==0 )
				{
					printf("\n      AMC-[%d%%]%d/%d. ",pEmPara->iSumBitOffset*100/8/(pEmPara->iMesLen+2),pEmPara->iSumBitOffset/8,pEmPara->iMesLen+2);
					printf("Embedding Rate=%d%% Hit Rate=%d C=%d.\n",pEmPara->iEmRate,pEmPara->iHitRate,pEmPara->ThresholdSm);
				}
				//添加嵌入进度
...
enc_stop(void *enc_handle)
{
	...
	xerr = xvid_encore(enc_handle, XVID_ENC_DESTROY, NULL, NULL,NULL);
