...
#include <math.h>	//添加log函数
#include <conio.h>	//添加getch
...
#include "../utils/mbfunctions.h"//添加 estimation_povp.c 中定义的 MBTransQuantInter();
#include "../image/qpel.h"//添加interpolate16x16_quarterpel函数
...
static __inline void
compensate16x16_interpolate(int16_t * const cur_block,
							...
							const int32_t rounding)
{
	...
	if(quarterpel) {
		if ((dx&3) | (dy&3)) {
			...
			ptr = tmp;
		} else ptr =  ref + ((int)y + dy/4)*(int)stride + (int)x + dx/4; /* fullpixel position *///获取参考块指针

	} else ptr = get_ref(ref, refh, refv, refhv, x, y, 1, dx, dy, stride);//获取参考块指针
...
/*	transfer_8to16sub(dct_codes, cur + y * stride + x,ptr, stride);
	...
	for (j = 0; j < 8; j++)
	{
		for (i = 0; i < 8; i++) 
		{
			const uint8_t c = cur[j * stride + i+ y * stride + x];
			cur_block[j * 8 + i] = (int16_t) ptr[j * stride + i];
			dct_codes[j * 8 + i] = (int16_t) c - (int16_t) ptr[j * stride + i];
		}
	}
	...
	for (j = 0; j < 8; j++)
	{
		for (i = 0; i < 8; i++) 
		{
			const uint8_t c = cur[j * stride + i+ y * stride + x + 8];
			cur_block[j * 8 + i+64] = (int16_t) ptr[j * stride + i +8] ;
			dct_codes[j * 8 + i+64] = (int16_t) c - (int16_t) ptr[j * stride + i +8];
		}
	}
	...
	for (j = 0; j < 8; j++)
	{
		for (i = 0; i < 8; i++) 
		{
			const uint8_t c = cur[j * stride + i+ y * stride + x + 8*stride];
			cur_block[j * 8 + i+128] = (int16_t) ptr[j * stride + i+ 8*stride];
			dct_codes[j * 8 + i+128] = (int16_t) c - (int16_t) ptr[j * stride + i+ 8*stride];
		}
	}
	...
	for (j = 0; j < 8; j++)
	{
		for (i = 0; i < 8; i++) 
		{
			const uint8_t c = cur[j * stride + i+ y * stride + x + 8*stride+8];
			cur_block[j * 8 + i+192] = (int16_t)ptr[j * stride + i+ 8*stride + 8];
			dct_codes[j * 8 + i+192] = (int16_t) c - (int16_t)ptr[j * stride + i+ 8*stride + 8];
		}
	}
}
...
static int CalComplexity(short* pCurBlock, int size)//计算当前块的平滑度
{
	int64_t sum = 0; //SSD
	int i,j;
	short block[16][16];
	...
	for (j = 0; j < 8; j++)
		for (i = 0; i < 8; i++)//将block拷贝到一个二维数组中
		{
			block[j][i]=pCurBlock[64*0+8*j+i];
			block[j][i+8]=pCurBlock[64*1+8*j+i];
			block[j+8][i]=pCurBlock[64*2+8*j+i];
			block[j+8][i+8]=pCurBlock[64*3+8*j+i];
		}
	...
	if(size == 8)
	{
		for (j = 0; j < 8; j++)
			for (i = 0; i < 7; i++) 			
				sum+=(block[j][i]-block[j][i+1])*(block[j][i]-block[j][i+1]);			

		for (j = 0; j < 7; j++)
			for (i = 0; i < 8; i++) 			
				sum+=(block[j][i]-block[j+1][i])*(block[j][i]-block[j+1][i]);			
	}
	else //size=16
	{
		for (j = 0; j < 16; j++)
...
/* main loop. checking all predictions (but first, which is 0,0 and has been checked in MotionEstimation())*/
	...
	for (i = 1; i < 7; i++)
		if (!vector_repeats(pmv, i)) {
			CheckCandidate(pmv[i].x, pmv[i].y, Data, i);	//逐一检查mpv[1-6]这六个最可能运动向量，如果发现他们与以前的运动不同，就调用CheckCandidate做运动估计
	...
	if ((Data->iMinSAD[0] <= threshA) ||
			...
			(Data->iMinSAD[0] < (prevMBs+x+y*pParam->mb_width)->sad16)))//如果当前最优运动向量，即Data->iMinSAD[0]，小于threshA？
																																//或者当前最优运动向量等于参考帧相同位置宏块的运动向量，并且对应的SAD值又比他的小？  
																																//就不再做inter4v的搜索  
		inter4v = 0;
	else {
...
void
MotionEstimation(Encoder* pEnc,MBParam * const pParam,
				 ...
				 EmbeddingPara* pEmPara)//添加嵌入参数)
{
	...
	DECLARE_ALIGNED_MATRIX(current_block, 6, 64, int16_t, CACHE_LINE);
	DECLARE_ALIGNED_MATRIX(qcoeff2, 6, 64, int16_t, CACHE_LINE);
	...
	for (y = 0; y < mb_height; y++)	{
		...
		for (x = 0; x < mb_width; x++)	{
			...
			//////////////////////// 添加参考块重建和当前宏块的重建，并计算PSNR //////////////////////////

			if(pEmPara && pEmPara->iFirstTime==0)
			{
				int i;
				pEmPara->iMvCandidateXY[0]=0;

				//================================= ( x , y ) ==================================

				data = Data;

				quarterpel=current->vol_flags & XVID_VOL_QUARTERPEL;

				MB = current->mbs[x + y * pParam->mb_width];
				MB.mvs[0] = MB.mvs[1] = MB.mvs[2] = MB.mvs[3] = data.currentMV[0];
				//原始运动矢量

				MB.pmvs[0].x = data.currentMV[0].x - data.predMV.x;
				MB.pmvs[0].y = data.currentMV[0].y - data.predMV.y;

				dx = (quarterpel ? MB.qmvs[0].x : MB.mvs[0].x);
				dy = (quarterpel ? MB.qmvs[0].y : MB.mvs[0].y);
				//计算运动向量dx，dy 

				compensate16x16_interpolate(&current_block[0 * 64],&dct_codes[0 * 64], current->image.y, reference->image.y, pEnc->vInterH.y,
									pEnc->vInterV.y, pEnc->vInterHV.y, pEnc->vInterV.u, 16 * x, 16 * y, dx, dy,
									pParam->edged_width, quarterpel, current->rounding_type);
				//调用compensate16x16_interpolate进行16*16大宏块的运动补偿

				MB.cbp = MBTransQuantInter2(pParam, current, &MB, x, y,
												dct_codes, qcoeff2);
				//得到反量化后的残差
				for(i=0;i<384;i++)
					current_block[i]=current_block[i]+dct_codes[i];
				//得到重建块

				sm[0] = CalComplexity(&current_block[64*0],16);
				//计算块的光滑度

				if(sm[0]<=pEmPara->ThresholdSm || pEmPara->iSumBitOffset<48)
					pEmPara->iMvCandidateXY[0]=1;
				else pEmPara->iMvCandidateXY[0]=0;
				//达到阈值，则设置候选标志
				...
				if(data.currentMV[0].x<=pEmPara->EX)
						data.currentMV[0].x++;
				else data.currentMV[0].x--;
				//x被修改
				...
				if(sm[0]<=pEmPara->ThresholdSm || pEmPara->iSumBitOffset<48)
					pEmPara->iMvCandidate_XY[0]=1;
				else pEmPara->iMvCandidate_XY[0]=0;
				//达到阈值，则设置候选标志
				...
				if(data.currentMV[0].y<=pEmPara->EY)
						data.currentMV[0].y++;
				else data.currentMV[0].y--;
				//y被修改
				...
				if(sm[0]<=pEmPara->ThresholdSm || pEmPara->iSumBitOffset<48)
					pEmPara->iMvCandidateX_Y[0]=1;
				else pEmPara->iMvCandidateX_Y[0]=0;
				//达到阈值，则设置候选标志
				...
				if(sm[0]<=pEmPara->ThresholdSm || pEmPara->iSumBitOffset<48)
					pEmPara->iMvCandidate_X_Y[0]=1;
				else pEmPara->iMvCandidate_X_Y[0]=0;
				//达到阈值，则设置候选标志
			}
			else if(pEmPara && pEmPara->iFirstTime)//第一次
			{
				//================================= ( x , y ) ==================================
				int i;
				data = Data;

				if(rand()%100<pEmPara->iEmRate)
				{
					int r=rand()%100;
					if(r<25)
						data.currentMV[0].x=data.currentMV[0].x+1;
					else if(r>=25&&r<50)
						data.currentMV[0].x=data.currentMV[0].x-1;

					r=rand()%100;
					if(r<25)
						data.currentMV[0].y=data.currentMV[0].y+1;
					else if(r>=25&&r<50)
						data.currentMV[0].y=data.currentMV[0].y-1;
				}
				...
				MB.cbp = MBTransQuantInter2(pParam, current, &MB, x, y,	dct_codes, qcoeff2);
				//得到反量化后的残差
				...
				pEmPara->iCurSm=sm[0];
				pEmPara->SmDump[sm[0]>9999?9999:sm[0]]++;

				if(pEmPara->iCurSm>9999)
					pEmPara->iCurSm=9999;
				//将这个SM暂时保存，如果将来这个宏块是inter，则作为统计之用
				//=======================================================================
			}
			...
			if (current->vop_flags & XVID_VOP_MODEDECISION_RD)
				xvid_me_ModeDecision_RD(&Data, pMB, pMBs, x, y, pParam,
								...
								pCurrent, pRef, pGMC, current->coding_type, bound,pEmPara);
	...
	for (y = (start_y + y_row); y < stop_y; y += y_step)	{
		...
		for (x = 0; x < mb_width; x++)	{
			...
			if (current->vop_flags & XVID_VOP_MODEDECISION_RD)
				xvid_me_ModeDecision_RD(&Data, pMB, pMBs, x, y, pParam,
								MotionFlags, current->vop_flags, current->vol_flags,
								pCurrent, pRef, pGMC, current->coding_type, bound,NULL);
