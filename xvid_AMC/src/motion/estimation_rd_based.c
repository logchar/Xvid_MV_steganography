...
static unsigned long Em_rand_seed;

void Em_mysrand (unsigned long seed)
{
    Em_rand_seed = seed;
}

unsigned long Em_myrand ()
{
	Em_rand_seed = (32342*Em_rand_seed+13232) %65535;
    return Em_rand_seed;
}
			 ...
			 EmbeddingPara* pEmPara)//添加嵌入参数
{
	...
	////////////////////////////// 手动添加变量 ////////////////////////////////
	int iMinSadXuyibo;
	SearchData DataXuyibo=*Data;
	VECTOR vOriVector;
	int iRate;
	
	vOriVector.x = Data->currentMV[0].x;	//保存原始的未改动的运动矢量
	vOriVector.y = Data->currentMV[0].y;	//保存原始的未改动的运动矢量

	if(pEmPara && pEmPara->iFirstTime==0 && pEmPara->iSranded==0)//未初始化随机函数
	{
		Em_mysrand(pEmPara->iKey);
		pEmPara->iSranded=1;
	}

	if(pEmPara && pEmPara->iFirstTime==0 && pEmPara->iCompleted==0 && pEmPara->iSranded)
		iRate = Em_myrand()%100;//随机函数
	...
	if(pEmPara && pEmPara->iFirstTime)//第一次压缩过程用于获取嵌入容量
	{
		pEmPara->iSumBlockNum++;

		/*if(rand()%100<pEmPara->iEmRate && pEmPara->Sm<1000)
		{
			int r=rand()%100;
			if(r<25)
	...
	else if(pEmPara && pEmPara->iFirstTime==0 && pEmPara->iCompleted==0 && (pEmPara->iMvCandidateXY[0] || pEmPara->iSumBitOffset<48))
	//第二次压缩过程，且当前块符合嵌入条件
	{
		iMinSadXuyibo = Data->iMinSAD[0];
		//保存原始sad

		*Data = DataXuyibo;
		//恢复原始Data

		if(pEmPara->iSumBitOffset==48)
			pEmPara->iSumBitOffset=48;

		if(pEmPara && pEmPara->pMesName)
		{
			pEmPara->iXEmbeded=0;
			pEmPara->iYEmbeded=0;

			if(iRate<pEmPara->iHitRate||pEmPara->iSumBitOffset<48)
			{
				//如果当前运动矢量大于一定的阈值，则可以进行秘密信息嵌入
				unsigned char MesDump;
				MesDump=pEmPara->InfDump[pEmPara->iSumBitOffset/8];

				if((abs(Data->currentMV[0].x)&0x01)!=((MesDump>>(7-pEmPara->iSumBitOffset%8))&0x01))//LSB与嵌入比特不同
				{
			...
			if(iRate<pEmPara->iHitRate||pEmPara->iSumBitOffset<48)
			{
				...
				if((abs(Data->currentMV[0].y)&0x01)!=((MesDump>>(7-pEmPara->iSumBitOffset%8))&0x01))//LSB与嵌入比特不同
				{
		...
		if(vOriVector.x == Data->currentMV[0].x && vOriVector.y == Data->currentMV[0].y && pEmPara->iMvCandidateXY[0] == 0)//x,y最低位都未被改变
		{
			//嵌入后SM变得比较高了，则此嵌入无效
			if(pEmPara->iXEmbeded){ pEmPara->iSumBitOffset--; pEmPara->iXEmbeded = 0; }
			if(pEmPara->iYEmbeded){ pEmPara->iSumBitOffset--; pEmPara->iYEmbeded = 0; }
		}

		if(vOriVector.x != Data->currentMV[0].x && vOriVector.y == Data->currentMV[0].y && pEmPara->iMvCandidate_XY[0] == 0)//x最低位被取反,y没被改动
		{
			//嵌入后SM变得比较高了，则此嵌入无效
			if(pEmPara->iXEmbeded){ pEmPara->iSumBitOffset--; pEmPara->iXEmbeded = 0; }
			if(pEmPara->iYEmbeded){ pEmPara->iSumBitOffset--; pEmPara->iYEmbeded = 0; }
		...
		if(vOriVector.x == Data->currentMV[0].x && vOriVector.y != Data->currentMV[0].y && pEmPara->iMvCandidateX_Y[0] == 0)//x没被改动,y最低位被取反
		{
			//嵌入后SM得比较高了，则此嵌入无效
			if(pEmPara->iXEmbeded){ pEmPara->iSumBitOffset--; pEmPara->iXEmbeded = 0; }
			if(pEmPara->iYEmbeded){ pEmPara->iSumBitOffset--; pEmPara->iYEmbeded = 0; }
		...
		if(vOriVector.x != Data->currentMV[0].x && vOriVector.y != Data->currentMV[0].y && pEmPara->iMvCandidate_X_Y[0] == 0)//x,y最低位都被取反
		{
			//嵌入后SM变得比较高了，则此嵌入无效
			if(pEmPara->iXEmbeded){ pEmPara->iSumBitOffset--; pEmPara->iXEmbeded = 0; }
			if(pEmPara->iYEmbeded){ pEmPara->iSumBitOffset--; pEmPara->iYEmbeded = 0; }
		...
		Data->iMinSAD[0] = iMinSadXuyibo;
		//丢弃修改向量带来的新sad，恢复原始sad
	}
	////////////////////////////////////////////////////////////////////////////
				...
				EmbeddingPara* pEmPara)//添加嵌入参数
{
	...
	///////////////////////// 目前向量修改的处理在此完成 ///////////////////////////
	min_rd = findRD_inter(Data, x, y, pParam, MotionFlags,pEmPara);//计算帧间预测的代价和
	//////////////////////////////////////////////////////////////////////////////////////
	...
	if (inter4v) {
		...
		v4_rd = 100000;//添加 将inter4v代价设置为最大这样就可以将inter4v宏块去除
		if (v4_rd < min_rd) {
...
//-------------------------------------- 添加回溯处理 -------------------------------------
	if(mode == MODE_INTER)
	{
		pEmPara->Sum_X+=Data->currentMV[0].x;
		pEmPara->Sum_Y+=Data->currentMV[0].y;
		pEmPara->num++;
		//添加运动矢量均值计算
		pEmPara->EX=pEmPara->Sum_X/pEmPara->num;
		pEmPara->EY=pEmPara->Sum_Y/pEmPara->num;
		//计算运动矢量均值
	}
	//if(((mode != MODE_INTER) || (mode == MODE_INTER && cbp <=3 && Data->currentMV[0].x==0 && Data->currentMV[0].y==0)) && pEmPara->iFirstTime)//第一次压缩过程用于获取嵌入容量
	if(mode != MODE_INTER && pEmPara->iFirstTime)//第一次压缩过程用于获取嵌入容量
	{
		//当初这个宏块作为了候选，但是现在这个宏块失效了
		if(pEmPara->iMvCandidateXY[0])
		{
			pEmPara->iBlockNum--;
			pEmPara->SmDump[pEmPara->iCurSm]--;
			//有效宏块数目以及sm的计数器减一，即回溯
		}
		pEmPara->iSumBlockNum--;
	}

	if (mode != MODE_INTER && pEmPara->iFirstTime == 0)//没有采用16*16帧间预测，并且是第二次压缩过程
	{
		if(pEmPara->iXEmbeded) pEmPara->iSumBitOffset--;
		if(pEmPara->iYEmbeded) pEmPara->iSumBitOffset--;
	}

	if(pEmPara && mode == MODE_INTER && pEmPara->iFirstTime == 0 && pEmPara->iCompleted==0)//采用16*16帧间预测，并且是第二次压缩过程
	{
		if((pEmPara->iXEmbeded || pEmPara->iYEmbeded) && pEmPara->iSumBitOffset%8==0)//存在有效嵌入,每嵌入8bit，更新下进度
		{
			if(pEmPara->iSumBitOffset/8 == pEmPara->iMesLen+2)
				pEmPara->iCompleted = 1;
		}
	}
			...
			const int bound)
{
	...
	if ((Data->iMinSAD[0] + 75 < Data->iMinSAD[1] +
		...
		min_rd = findRD_inter(Data, x, y, pParam, MotionFlags,NULL);
