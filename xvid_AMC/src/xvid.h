...
typedef struct
{
	unsigned char Dump[10];		//提取秘密信息的缓存
	int iKey;					//提取密钥
	unsigned int iSumBitOffset;	//所有信息（包括参数信息）寻列的bit偏移标记
	unsigned int  iMesLen;		//秘密信息文件（不包括参数信息）字节数
	char *pMesFileName;			//需要嵌入的秘密信息文件名称
	int iXVector;				//运动矢量X分量
	int iYVector;				//运动矢量Y分量
	int iInter;					//帧间预测标志
	int iSranded;				//已经进行了随机函数初始化标志
	int iCandidateMv;
	int iHitRate;				//命中率，决定相对嵌入率
	int iSm;					//宏块复杂度参数阈值SM
}ExtractPara;
