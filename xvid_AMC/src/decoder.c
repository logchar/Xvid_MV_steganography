...
////////////////////////////////////////添加提取相关的变量跟随机函数///////////////////////////////////////////////
static unsigned long Ex_rand_seed;

void Ex_mysrand (unsigned long seed)
{
    Ex_rand_seed = seed;
}

unsigned long Ex_myrand ()
{
    Ex_rand_seed = (32342*Ex_rand_seed+13232) %65535;
    return Ex_rand_seed;
}
		...
		ExtractPara* pExPara)//添加提取参数
{
  ...
  if ((pMB->mode != MODE_INTER4V) || (bvop)) { /* INTER, INTER_Q, NOT_CODED, FORWARD, BACKWARD */
    ...
    uv_dy = mv[0].y;
/////////////////////////////////////// 添加运动矢量提取代码 /////////////////////////////////////////
	if(pExPara)// && pExPara->pVectorFileName[0]!='\0' && pExPara->pMesFileName[0]!='\0'
	{
		pExPara->iInter=1;
		pExPara->iXVector=uv_dx;
		pExPara->iYVector=uv_dy;
		//保存这个块的运动矢量
	}
////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		ExtractPara* pExPara)//添加提取参数
{
  ...
  ///////////////////////////// 添加变量用于秘密信息提取 //////////////////////////////////
	int iHitProb;		//添加概率命中的变量，实现密钥决定嵌入位置
	FILE* Mfp=NULL;		//信息文件
	FILE* Vfp=NULL;		//运动矢量保存文件
	uint8_t *pY_Cur;	//当前16*16亮度块首地址
	short CurBlock[256];//当前16*16亮度块存放到这个一维数组中
	uint32_t stride = dec->edged_width;//获得帧宽度
	int i,j;			//8*8块内定位
	int sm;				//存放当前宏块SM，将用来与SM阈值做比较
   ///////////////////////////////////////////////////////////////////////////////////////////////
...
for (y = 0; y < mb_height; y++) {
    ...
    for (x = 0; x < mb_width; x++) {
      MACROBLOCK *mb;
		...
		/////////////////////////////添加随机函数初始化///////////////////////////
  		if(pExPara && pExPara->iSranded==0)//未初始化随机函数
		{
			Ex_mysrand(pExPara->iKey);
			pExPara->iSranded=1;
		}

		if(pExPara && pExPara->iSranded)
		{
			if(Ex_myrand()%100<pExPara->iHitRate || pExPara->iSumBitOffset<48)//概率命中			
				iHitProb=1;
			else
				iHitProb=0;
		}
      ...
      if (!(BitstreamGetBit(bs))) { /* block _is_ coded */
		...
		{
			//if(x==4&&y==7)
			//pExPara->iSumBitOffset=pExPara->iSumBitOffset;
			decoder_mbinter(dec, mb, x, y, cbp, bs, rounding, 0, 0,pExPara);
			//解码
			///////////////////////////////////// 添加秘密信息提取 /////////////////////////////////////////
			if(pExPara && pExPara->pMesFileName)
			{
				pY_Cur = dec->cur.y + (y << 4) * stride + (x << 4);//计算当前块所在当前帧位置

				for(j=0;j<8;j++)
					for(i=0;i<8;i++)
						CurBlock[j*8+i]=pY_Cur[j*stride+i];//
				for(j=0;j<8;j++)
					for(i=0;i<8;i++)
						CurBlock[j*8+i+64]=pY_Cur[j*stride+i+8];
				for(j=0;j<8;j++)
					for(i=0;i<8;i++)
						CurBlock[j*8+i+128]=pY_Cur[j*stride+i+8*stride];
				for(j=0;j<8;j++)
					for(i=0;i<8;i++)
						CurBlock[j*8+i+192]=pY_Cur[j*stride+i+8*stride+8];//
				//依次拷贝四个8*8亮度块到一位数组CurBlock中，后面将计算SM
				sm=CalComplexity(CurBlock,16);
			}//计算一下这个块的SM

			if(pExPara && pExPara->pMesFileName && pExPara->iInter && iHitProb)
			{
				//提取一个SM符合阈值的inter宏块
				unsigned int iMask;

				iMask = abs(pExPara->iXVector)%2;

				if(pExPara->iSumBitOffset<8)//0-7bit，记录 Hit Rate
				{
					iMask = iMask << (7-pExPara->iSumBitOffset);
					pExPara->iHitRate+=iMask;
					pExPara->iSumBitOffset++;
				}
				else if(pExPara->iSumBitOffset<32)//处于秘密信息长度参数提取阶段
				{
					iMask = iMask << (31-pExPara->iSumBitOffset);
					pExPara->iMesLen+=iMask;
					pExPara->iSumBitOffset++;
				}
				else if(pExPara->iSumBitOffset<48)//处于SM参数提取阶段
				{
					if(pExPara->iSumBitOffset==32)//
						pExPara->iSm=0;
					iMask = iMask << (47-pExPara->iSumBitOffset);
					pExPara->iSm+=iMask;
					pExPara->iSumBitOffset++;
				}
				else if(pExPara->iSumBitOffset<(pExPara->iMesLen*8+48) && sm<=pExPara->iSm)//处于秘密信息数据提取阶段
				{
					iMask=iMask<<(7-pExPara->iSumBitOffset % 8);
					pExPara->Dump[0]+=iMask;//将这个bit放入缓存
					pExPara->iSumBitOffset++;
					if(pExPara->iSumBitOffset % 8 == 0)//一个字节的缓存已满
					{
						Mfp=fopen(pExPara->pMesFileName,"ab+");//打开秘密信息输出文件
						if(Mfp)
						{
							fwrite(pExPara->Dump,1,1,Mfp);
							pExPara->Dump[0]=0;
							fclose(Mfp);
						}
					}
				...
				else if(pExPara->iSumBitOffset<32)//还处于秘密参数提取阶段
				{
					iMask = iMask << (31-pExPara->iSumBitOffset);
					pExPara->iMesLen+=iMask;
					pExPara->iSumBitOffset++;
		...
		//pExPara->iSm=250;
		if(pExPara->iSumBitOffset==112)
			pExPara->iSumBitOffset=112;
		...
		if(pExPara && pExPara->pMesFileName && pExPara->iInter && iHitProb)
		{
			...
			else if(pExPara->iSumBitOffset<(pExPara->iMesLen*8+32) && sm<=pExPara->iSm)//处于秘密信息数据提取阶段
			{
				iMask=iMask<<(7-pExPara->iSumBitOffset % 8);
				pExPara->Dump[0]+=iMask;//将这个bit放入缓存
				pExPara->iSumBitOffset++;
  ...
  for (y = 0; y < dec->mb_height; y++) {
    ...
    for (x = 0; x < dec->mb_width; x++) {
      ...
      if (last_mb->mode == MODE_NOT_CODED) {
        ...
        decoder_mbinter(dec, mb, x, y, mb->cbp, bs, 0, 1, 1,NULL);
        continue;
      }
      ...
      case MODE_BACKWARD:
        ...
        decoder_mbinter(dec, mb, x, y, mb->cbp, bs, 0, 0, 1,NULL);
        break;
  ...
  if (dec->packed_mode && coding_type == N_VOP) {
    ...
    case P_VOP :
      decoder_pframe(dec, &bs, rounding, quant,
                        fcode_forward, intra_dc_threshold, NULL,pExPara);
      break;
    case S_VOP :
      decoder_pframe(dec, &bs, rounding, quant,
                        fcode_forward, intra_dc_threshold, &gmc_warp,pExPara);
      break;
    case N_VOP :
