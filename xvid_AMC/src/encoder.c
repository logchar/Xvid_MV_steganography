...
repeat:
	 ...
	 * %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% */
		...
		if ( FrameCodeP(pEnc, &bs,pEmPara) == 0 ) {/////////////// P 帧编码都在这里进行////////////////
				...
				if (current->coding_type == P_VOP) {	/* special rule for P-VOP's SKIP */
					for (k = pEnc->bframenum_head; k < pEnc->bframenum_tail; k++) {
						...
						if (iSAD >= BFRAME_SKIP_THRESHHOLD * pMB->quant || ((bound > 1) &&
							...
							if (pParam->vol_flags & XVID_VOL_QUARTERPEL) {//4分之一像素
...
static int
FrameCodeP(Encoder * pEnc, Bitstream * bs,EmbeddingPara* pEmPara)//添加嵌入参数
{
	int bits = BitstreamPos(bs);
...
#endif
	{
		...
		MotionEstimation(pEnc,&pEnc->mbParam, current, reference,
